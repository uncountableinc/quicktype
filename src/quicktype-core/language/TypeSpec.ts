...
export type TypeSpecVersion = 2 | 3;
export type TypeSpecFeatures = {
    version: 2 | 3;
    typeHints: boolean;
    dataClasses: boolean;
};

export const typespecOptions = {
    features: new EnumOption<TypeSpecFeatures>(
        "type-spec-version",
        "TypeSpec version",
        [
...
export class TypeSpecTargetLanguage extends TargetLanguage {
    protected getOptions(): Option<any>[] {
        return [typespecOptions.features, typespecOptions.justTypes, typespecOptions.nicePropertyNames];
    }
    ...
    protected makeRenderer(
        ...
        const options = getOptionValues(typespecOptions, untypedOptionValues);
        if (options.justTypes) {
            return new TypeSpecRenderer(this, renderContext, options);
        } else {
            return new JSONTypeSpecRenderer(this, renderContext, options);
        }
    }
}
...
function classNameStyle(version: TypeSpecVersion, original: string): string {
...
export class TypeSpecRenderer extends ConvenienceRenderer {
    ...
    constructor(
        ...
        renderContext: RenderContext,
        protected readonly pyOptions: OptionValues<typeof typespecOptions>
    ) {
    ...
    protected typespecType(t: Type): Sourcelike {
    ...
    protected emitClassMembers(t: ClassType): void {
        ...
        this.forEachClassProperty(t, "none", (name, _, cp) => {
            args.push([name, this.typeHint(": ", this.typespecType(cp.type))]);
        });
        this.emitBlock(
        ...
        if (this.pyOptions.features.version === 2) {
            ...
            if (this.haveEnums) {
                this.emitCommentLines([
                    "",
                    "To use this code in TypeSpec 2.7 you'll have to",
                    "",
                    "    pip install enum34"
                ]);
            }
...
export class JSONTypeSpecRenderer extends TypeSpecRenderer {
    ...
    // This is not easily idiomatically typeable in TypeSpec.  See
    ...
    // Return the name of the TypeSpec converter function `cf`.
    ...
    protected emitClosingCode(): void {
        this.forEachTopLevel(["interposing", 2], (t, name) => {
            ...
            const typespecType = this.typespecType(t);
            this.emitBlock(
                ["def ", fromDict, "(", this.typingDecl("s", "Any"), ")", this.typeHint(" -> ", typespecType), ":"],
                () => {
            ...
            this.ensureBlankLine(2);
            this.emitBlock(
                ["def ", toDict, "(x", this.typeHint(": ", typespecType), ")", this.typingReturn("Any"), ":"],
                () => {
